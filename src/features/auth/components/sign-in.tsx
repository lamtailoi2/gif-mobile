...
export const SignIn = () => {
  ...
  const onGooglePress = async () => {
    setError("");
    try {
      const { createdSessionId, setActive: setSession } = await startSSOFlow({
  ...
  const onSignIn = async () => {
    ...
    if (signInError) {
      setError(
        signInError?.longMessage ?? signInError?.message ?? "Sign in failed",
      );
      return;
    }
  ...
  return (
    ...
    >
      <SafeAreaView className="flex-1 justify-center items-center px-6">
        <ThemedView
          className="w-full max-w-[400] gap-4 p-6"
          style={{
        ...
        >
          ...
          {!!error && (
            <ThemedText className="text-[#FF3B30] text-center">
              {error}
            </ThemedText>
          )}
          <Input
